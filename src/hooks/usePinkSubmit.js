...
const usePinkSubmit = (id) => {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      // console.log("pinkData", pinkData);

      // setIsSubmitting(false);
      // return;

      if (id) {
        const res = await PinkServices.updatePink(id, pinkData);
        setIsUpdate(true);
        setIsSubmitting(false);
        notifySuccess(res.message);
        closeDrawer();
      } else {
        const res = await PinkServices.addPink(pinkData);
        setIsUpdate(true);
        setIsSubmitting(false);
        notifySuccess(res.message);
        closeDrawer();
  ...
  useEffect(() => {
    ...
    if (id) {
      (async () => {
        try {
          const res = await PinkServices.getPinkById(id);
          if (res) {
            // console.log('res pink', res);
            setResData(res);
